...
function general_usage(){
 echo "Usage:"
  echo "  ./pgscalculator.sh --config <file> --steps <steps> [options]"
 echo ""
  echo "Required:"
 ...
 echo "Examples:"
  ...
  echo "  ./pgscalculator.sh --config config.yaml --steps prep"
 echo ""
  echo "  # Step 2: Run per-sumstat steps"
  echo "  ./pgscalculator.sh --config config.yaml --steps sumstat,weights,score,finalize -i /path/to/sumstat_814"
 echo ""
  echo "  # Or submit as SLURM jobs"
  echo "  ./pgscalculator.sh --config config.yaml --steps prep --sbatch"
  echo "  ./pgscalculator.sh --config config.yaml --steps sumstat,weights,score,finalize -i /path/to/sumstat_814 --sbatch"
}
...
if [[ -z "$steps_arg" ]]; then
  ...
  >&2 echo ""
  >&2 echo "Examples:"
  >&2 echo "  ./pgscalculator.sh --config config.yaml --steps prep"
  >&2 echo "  ./pgscalculator.sh --config config.yaml --steps sumstat,weights,score,finalize -i /path/to/sumstat"
  exit 1
fi
...
infold_host=$(realpath "${infold}")
  ...
  submit_array_for_step() {
    ...
    run_cmd="${project_dir}/pgscalculator.sh --config ${config_file_host} --steps ${steps_arg_for_task}"
...
rc=\$?; echo \"[INFO] Finished ${step_profile} chr\${CHR} at \$(date) (exit=\$rc)\"; exit \$rc"
  ...
  submit_finalize_job() {
    ...
    run_cmd="${project_dir}/pgscalculator.sh --config ${config_file_host} --steps combine-scores,finalize-output"
    [[ -n "$infold" ]] && run_cmd="${run_cmd} -i ${infold}"
    [[ -n "$outdir" ]] && run_cmd="${run_cmd} -o ${outdir}"
    [[ -n "$devmode" ]] && run_cmd="${run_cmd} -d"
    [[ -n "$force_mode" ]] && run_cmd="${run_cmd} --force"
  ...
  run_base="${project_dir}/pgscalculator.sh --config ${config_file_host}"
...
if [[ "$use_sbatch" == true ]]; then
  ...
  if [[ "$has_prep" == true ]]; then
    ...
    run_cmd="${project_dir}/pgscalculator.sh --config ${config_file_host} --steps prep --_driver-run"
    [[ -n "$outdir" ]] && run_cmd="${run_cmd} -o ${outdir}"
    [[ -n "$devmode" ]] && run_cmd="${run_cmd} -d"
    [[ -n "$force_mode" ]] && run_cmd="${run_cmd} --force"
  ...
  run_cmd="${project_dir}/pgscalculator.sh --config ${config_file_host} --steps ${steps_arg} -i ${infold} --_driver-run"
  [[ -n "$outdir" ]] && run_cmd="${run_cmd} -o ${outdir}"
  [[ -n "$devmode" ]] && run_cmd="${run_cmd} -d"
  [[ -n "$force_mode" ]] && run_cmd="${run_cmd} --force"
  [[ "$do_cleanup" == true ]] && run_cmd="${run_cmd} --cleanup"
...
if [[ "$needs_prep_check" -eq 1 ]]; then
  ...
  if ! missing_prep=$(check_prep_exists "$outdir_host"); then
    ...
    >&2 echo "  ./pgscalculator.sh --config ${config_file} --steps prep"
    >&2 echo ""
...
if [[ "$steps_arg" == *"weights"* ]] && [[ "$steps_arg" != *"sumstat"* ]]; then
  if ! missing_sumstat=$(check_sumstat_exists "$outdir_host" "$sumstat_name"); then
    ...
    >&2 echo "  ./pgscalculator.sh --config ${config_file} --steps sumstat -i ${infold}"
    >&2 echo ""
    >&2 echo "Or include sumstat in your steps:"
    >&2 echo "  ./pgscalculator.sh --config ${config_file} --steps sumstat,weights -i ${infold}"
    exit 1
  fi
fi
...
if [[ "$steps_arg" == *"score"* ]] && [[ "$steps_arg" != *"weights"* ]]; then
  if ! missing_posteriors=$(check_posteriors_exists "$outdir_host" "$sumstat_name"); then
    ...
    >&2 echo "  ./pgscalculator.sh --config ${config_file} --steps weights -i ${infold}"
    >&2 echo ""
    >&2 echo "Or include weights in your steps:"
    >&2 echo "  ./pgscalculator.sh --config ${config_file} --steps weights,score -i ${infold}"
  exit 1
  fi
fi
