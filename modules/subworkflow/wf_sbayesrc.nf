...
include { 
 calc_posteriors_sbayesrc 
 concatenate_sbayes_posteriors
 qc_posteriors
 format_sbayesrc_posteriors
...
workflow wf_sbayesrc_calc_posteriors {
  ...
  // TODO: Add annotation files handling when annotation data is available
  // For now, create a placeholder channel
  Channel
  .from(1..22)
  .map { chr -> 
    // TODO: Replace this with actual annotation file paths
    def annotFile = file("/dev/null") // Placeholder - will need real annotation files
    return tuple(chr.toString(), annotFile)
  }
  .set { ch_annotations }
  ...
  // Read all genotype files and detect format (same as sbayesR)
  ...
  // Split sumstat per chromosome (same as sbayesR)
  ...
  // format chr-chunked sumstats (same as sbayesR)
  ...
  // join for variant map (same as sbayesR but will also include annotations)
  ...
  // make variant map (reuse sbayesR variant map process)
  ...
  // Formatting according to sbayesRC (reuse sbayesR formatting)
  format_sumstats(rmcol_build_sumstats.out, mapfile, "sbayesr")
  format_sumstats.out.set { sumstats }

  // Join sumstats, LD files, and annotations for sbayesRC
  sumstats
  .join(ch_ldfiles)
  .join(ch_annotations)
  .set { ch_calc_posteriors }  

  // Calculate posteriors using sbayesRC
  calc_posteriors_sbayesrc(ch_calc_posteriors).set { ch_calculated_posteriors }

  //format and id-mapping for scoring
  ch_calculated_posteriors
  .join(variant_map_for_sbayesr.out.map)
  .set { ch_format_posteriors }  
  format_sbayesrc_posteriors(ch_format_posteriors).set { ch_formatted_posteriors }
...
workflow wf_sbayesrc_calc_score {
  ...
  // Use the unified PLINK2 files for scoring (same as sbayesR)
  unified_plink2_files
  .map { 
    if (params.dev) {
      file("${params.outdir}/channel-trace").mkdirs()
  ...
  // Extract maf from genotypes (same as sbayesR)
  genotypes
  ...
  // prepare benchmark scoring (same as sbayesR)
  sumstat
  .join(variant_maps_for_sbayesr)
  ...
  // Mix main method and benchmark method after adding genotypes (same as sbayesR)
  ...
  // Calc score (same as sbayesR)
  ch_calc_score_input_per_chr
  .mix(ch_calc_score_input_all)
  .set{ ch_calc_score }
  calc_score(ch_calc_score)

  // Step 1: Branch off and prepare for collection (same as sbayesR)
  ...
  // Merged chromosomes scores (same as sbayesR)
  calc_merged_score(ch_to_merge_collected)

  // Make Augmented GWAS (same as sbayesR)
  sumstat
  .join(extract_maf_from_genotypes.out)
